...
class MyGame extends MiniGame {
  constructor(message, client) {
    ...
    this.client.sendMessage(this.chatId, "Game has started ! Guess the number");
  }
...
class TTT extends MiniGame {
  constructor(message, client, P1, P2) {
    ...
    this.P1 = P1;
    this.P2 = P2;
    this.client.sendMessage(this.chatId, "Game has started ! Make your move" + this.P1.pushname);
    this.turn = 1;
    this.arr = ["", "", "", "", "", "", "", "", "", ""];
...
class MyGame2 extends MiniGame {
  constructor(message, client, question, answer) {
    ...
    this.client.sendMessage(this.chatId, "*Game has started ! Solve the riddle*\n\n" + "_" + this.question + "_");
  }
...
client.on('qr', (qr) => {
  console.log('Qr has been received', qr);
...
client.on("message_create", async (msg) => {
  ...
  // if(!msg.body.startsWith("!")){
  if (msg.body.includes("@") && !msg.body.startsWith("*Messages*") && !msg.body.includes("This message is automated by whats because of !all command")) {
  ...
  if (!msg.body.includes("_This message is automated by whats") && !msg.body.startsWith("*Messages*") && !msg.body.includes("To get more info use ```!help [command]```")) {
...
app.get("/", (req, res) => {
  res.send(
    '<h1>This server is powered by whats<br></h1>'
  );
});
